...
const products = [
  ...
  { name: "Bolsa", category: "Acessórios", stock: 10, price: 140.0, icon: "👜" },
...
const categorySummary: Record<string, number> = products.reduce((acc, product) => {
  acc[product.category] = (acc[product.category] || 0) + product.stock;
  return acc;
}, {} as Record<string, number>);

const [showFilterOptions, setShowFilterOptions] = useState<boolean>(false);
  const [selectedFilter, setSelectedFilter] = useState<FilterOption>("category");
...
return (
  <div className="flex min-h-screen">
    ...
    <div className="flex-1 bg-gray-100 p-4 sm:p-6 relative overflow-auto">
      ...
      <button
        className={`md:hidden absolute top-4 left-4 z-50 ${sidebarOpen ? "hidden" : ""}`}
        ...
        <main className="w-full p-4">
          <div className="flex flex-col md:flex-row justify-between items-center mb-4">
            <div className="text-center md:text-left mb-2 md:mb-0">
              ...
              <p className="text-base md:text-sm">{totalItems} Itens cadastrados</p>
              <p className="text-base md:text-sm">Quantidade disponível: {totalStock}</p>
            ...
            <form className="flex flex-nowrap sm:justify-end gap-2 w-full sm:max-w-md max-w-xs bg-white text-black border border-[--color-color-3] p-2 rounded-lg">
              ...
              <div className="relative inline-block text-left">
                ...
                {showFilterOptions && (
                  <div className="absolute top-full mt-1 w-48 bg-white border border-gray-300 rounded shadow z-10">
                    <p className="px-3 py-2 font-semibold border-b">Pesquisar por:</p>
                    <ul className="flex flex-col">
                      <li className="px-3 py-2 hover:bg-gray-100 cursor-pointer" onClick={() => handleFilterSelect("category")}>
                        Categoria
                      </li>
                    </ul>
                  </div>
              ...
              <button type="submit" className="border p-1.5 rounded text-base">
                Enviar
              </button>
            </form>
          </div>
            ...
            <table className="min-w-full bg-white border text-lg md:text-base">
              <tbody>
                {products.map((product, index) => (
                  <tr key={index} className={`text-center ${index % 2 === 0 ? "bg-color-1" : "bg-color-2"}`}>
                    ...
                    <td className="border px-4 py-3">R$ {product.price.toFixed(2)}</td>
